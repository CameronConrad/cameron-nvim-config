...
local plugins = {
  ...
  },
  {
    ...
    config = function()
      require('maven').setup({
        executable = "/opt/homebrew/bin/mvn", -- `mvn` should be in your `PATH`, or the path to the maven exectable, for example `./mvnw`
          ...
          }, 
          {
            { 
              ...
              entry_filter = function(entry, ctx)
                local other_entries = vim.tbl_filter(function(e)
                  return e.source.name ~= "buffer"
                end, cmp.get_entries())
                return #other_entries == 0 > 0
              end,
            },
          }
        ),
